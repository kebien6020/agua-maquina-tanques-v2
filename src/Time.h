...
inline constexpr auto operator==(Duration const& lhs, Duration const& rhs)
	-> bool {
...
class Timestamp {
   ...
   public:
	...
	friend constexpr auto operator-(Timestamp const&, Timestamp const&)
		-> Duration;
	friend constexpr auto operator+(Timestamp const&, Duration const&)
		-> Timestamp;

	constexpr explicit operator bool() { return value != 0ul; }
};

constexpr auto operator-(Timestamp const& lhs, Timestamp const& rhs)
	-> Duration {
	return {static_cast<long>(lhs.value - rhs.value)};
}

constexpr auto operator+(Timestamp const& stamp, Duration const& dur)
	-> Timestamp {
